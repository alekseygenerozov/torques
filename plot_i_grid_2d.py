...
num=np.zeros([len(e_test), len(ang_test)])
...
for ii,e1 in enumerate(e_test):
	for jj,ang in enumerate(ang_test):
		...
		deriv[ii,jj]=np.mean(prec_rate['{3}_a_e{0}_a{1}_o{2}_q-1.6_ein0.9_dt1.8'.format(e1, a1, ang, tag)])*norm
		derivb[ii,jj]=np.mean(prec_rate['{3}_b_e{0}_a{1}_o{2}_q-1.6_ein0.9_dt1.8'.format(e1, a1, ang, tag)])*norm
...
p1=ax.pcolormesh(ang_ords, e_ords, np.abs(deriv_avg/deriv_norm), cmap='RdBu_r',norm=colors.SymLogNorm(linthresh=100, vmin=-1.0e6, vmax=1.0e6))
